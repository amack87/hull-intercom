...
function sendLeads(ctx, payload) {
  ...
  leadsToSave.map(u =>
    ctx.client.asUser(u).logger.debug("outgoing.user.start")
  );
  const intercomLeadsToSave = leadsToSave.map(u =>
    syncAgent.userMapping.getIntercomLeadFields(u)
  );
  ...
  return syncAgent
    ...
    .then(res => {
      const savedleads = _.intersectionWith(
        leadsToSave,
        res,
        (lead, result) => {
        ...
        const intercomData = _.find(res, {
          user_id: u["traits_intercom/lead_user_id"]
        });
