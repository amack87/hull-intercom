    ...
    .catch(err => {
      if (_.get(err, "body.errors[0].code") === "scroll_exists") {
        client.logger.error("incoming.job.error", {
          ...
          errors: "Trying to perform two separate scrolls"
        });
        return Promise.resolve([]);
      }

      if (_.get(err, "body.errors[0].code") === "not_found") {
        client.logger.warn("incoming.job.warning", {
          jobName: "fetchAllLeads",
          errors: "Scroll expired, what could mean the end of the list"
        });
        return Promise.resolve([]);
      }
